...
from vectorize import add_vector, get_collection_stats, search_vector
...
def bot_rag_answer_message(history, question):
    ...
    if not doc_count:
        logger.warning(
            "RAG collection is not ready; answering with the default chat model"
        )
        fallback_messages = (
            [
                {
                    ...
                    "content": (
                        ...
                        "Kho dữ liệu RAG cục bộ chưa được import, nên hãy trả lời "
                        "dựa trên kiến thức chung và nói rõ khi cần kiểm tra căn cứ pháp lý."
                    ),
                }
            ]
            + history
