...
CHAT_API_KEY = GEMINI_API_KEY or OPENAI_API_KEY
CHAT_BASE_URL = (
...
def get_financial_agent_answer(messages, model=None, tools=None):
    if not tools:
...
def get_financial_agent_handle(messages, model=None, tools=None):
    if not tools:
        tools = get_financial_tools()
