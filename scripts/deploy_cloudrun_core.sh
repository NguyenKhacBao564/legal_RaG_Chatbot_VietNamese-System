...
CHAT_API_TIMEOUT="${CHAT_API_TIMEOUT:-120}"
EMBEDDING_API_TIMEOUT="${EMBEDDING_API_TIMEOUT:-60}"
BACKEND_ENV_VARS="GEMINI_BASE_URL=https://generativelanguage.googleapis.com/v1beta/openai/,GEMINI_MODEL=${GEMINI_MODEL:-gemini-2.5-flash},CHAT_API_TIMEOUT=$CHAT_API_TIMEOUT,VIETNAMESE_LLM_API_URL=,CUSTOM_EMBEDDING_ENABLED=false,FORCE_SYNC_CHAT=true,QDRANT_COLLECTION=$QDRANT_COLLECTION,QDRANT_VECTOR_SIZE=$QDRANT_VECTOR_SIZE,QDRANT_DISTANCE=$QDRANT_DISTANCE,EMBEDDING_BASE_URL=https://generativelanguage.googleapis.com/v1beta/openai/,EMBEDDING_MODEL=$EMBEDDING_MODEL,EMBEDDING_API_TIMEOUT=$EMBEDDING_API_TIMEOUT"
CLOUD_SQL_ARGS=()
SECRET_ARGS=()

GEMINI_SECRET="${GEMINI_SECRET:-GEMINI_API_KEY}"
QDRANT_SECRET="${QDRANT_SECRET:-QDRANT_API_KEY}"
MYSQL_PASSWORD_SECRET="${MYSQL_PASSWORD_SECRET:-MYSQL_PASSWORD}"

BACKEND_SECRETS="GEMINI_API_KEY=$GEMINI_SECRET:latest,GOOGLE_API_KEY=$GEMINI_SECRET:latest,EMBEDDING_API_KEY=$GEMINI_SECRET:latest,QDRANT_API_KEY=$QDRANT_SECRET:latest,MYSQL_PASSWORD=$MYSQL_PASSWORD_SECRET:latest"
SECRET_ARGS+=(--set-secrets "$BACKEND_SECRETS")
...
gcloud run deploy "$FRONTEND_SERVICE" \
  ...
  --set-env-vars API_BASE_URL="$BACKEND_URL",CHAT_SYNC_REQUEST=true,CHAT_STREAMING_ENABLED=false,STREAMLIT_SERVER_ADDRESS=0.0.0.0 \
  --project "$PROJECT_ID"
